...
impl<F: GpuField> Constraint<F> {
    ...
    //         let highest_degree_term = remainder[0].0.iter().map(|term|
    // &term.1).max().unwrap();         let highest_degree_variable =
    // highest_degree_term.0.iter().max_by_key(|v| v.1).unwrap();
    ...
    //             new_constraint = Constraint::from(old_element) *
    // Constraint::from(old_element);         } else {
    //             // no elements with power >2 so create a new column composed of
    //             // two elements i.e. reduce x * y * z -> w * z where w = x * y
    //             let old_elements = (highest_degree_term.0[0].0,
    // highest_degree_term.0[1].0);
    ...
    //                 for Term(_, variables) in &mut new_terms {
    //                     let elements = variables.0.iter().map(|v|
    // v.0).collect::<Vec<_>>();                     let mut new_variables =
    // variables.0.clone();

    //                     // check if the term degree can be reduced
    //                     if elements.contains(&old_elements.0) &&
    // elements.contains(&old_elements.1)                     {
    //                         new_variables
    //                             .retain(|v| v.0 != old_elements.0 && v.0 !=
    // old_elements.1);                         new_variables.push((new_element,
    // 1));                         *variables = Variables::new(new_variables);
    //                     }
    //                 }
    ...
    //             new_constraint =
    //                 Constraint::from(old_elements.0) *
    // Constraint::from(old_elements.1);         }

    //         res.push(are_eq(Constraint::from(new_element), &new_constraint));
    //         let col = Constraint::evaluate_symbolic(&[new_constraint], &[],
    // trace_step, trace_lde);         trace_lde.append(col);
    //     }
