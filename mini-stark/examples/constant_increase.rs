//! STARK proof and verification of a program that increases the number `7` by `+1` each cycle.
...
///
/// This is the function the stark proof will be generated over. Look at [transition_constraints] to see
/// the constraint that ensures the computational integrity of this function.
...
/// Generates the constraint to specify the number increases by '1' from one cycle to the next.
///
/// Constraints are expressed as multivariate polynomials. Variables represent register values in
/// either the current and next execution cycle.
fn transition_constraint() -> MultivariatePolynomial<PrimeFieldElement> {
...
///
/// The trace is an array representing the state of program every cycle. The state of the program at each cycle
/// is obtained by providing the values of all registers. For this program only a single register is needed.
/// It's technically an "algebraic" trace due to the fact that register values consist of finite field elements.
fn execute_program_with_trace(
    starting_number: u32,
    increment_times: u32,
) -> (u32, Vec<Vec<PrimeFieldElement>>) {
    let mut trace = Vec::with_capacity(increment_times as usize);

    // The starting state needs to be in the trace.
    let starting_register_values = vec![PrimeFieldElement::from(starting_number)];
    ...
    for _ in 1..increment_times {
        ...
        let next_register_values = vec![PrimeFieldElement::from(last_value)];
...
fn main() {
    ...
    // Execution and generation of the trace. Prover would provide the output along with
    // the STARK proof to the verifier. The program increments the number every cycle.
    // **Trace only needs to be known by the prover**.
