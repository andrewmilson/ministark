...
/// the second corresponds to the register and the third corresponds the value. The boundary constraints are public
...
fn transition_constraints(
    ...
    // field elements stored at a given context/address/clock cycle prior to the memory operation
    ...
    // field elements stored at a given context/address/clock cycle after the memory operation
    ...
    let d0 = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    let d1 = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    ...
    // field elements stored at a given context/address/clock cycle after the memory operation
    let v0_prime = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    let v1_prime = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    let v2_prime = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    let v3_prime = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    ...
    let t_prime = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    let perm_prod_prime = MultivariatePolynomial::<PrimeFieldElement>::lift(
        Polynomial::x(),
        increment(&mut column_counter),
    );
    ...
    // 1. `n0 = 1` - when the context changes, columns `d0` and `d1` at the next row should
    //    contain the delta between the old and the new contexts.
    let context_increase_constraint = n0.clone()
        * ((c_prime.clone() - c.clone())
            - (d0_prime.clone() + d1_prime.clone() * u16_values.clone()));
    // 2. `n0 = 0`, `n1 = 1` - when the context remains the same but the address changes, columns
    //    `d0` and `d1` at the next row should contain the delta between the old and the new addresses.
    let address_increase_constraint = (one.clone() - n0.clone())
        * (n1.clone())
        * ((a_prime.clone() - a.clone())
            - (d0_prime.clone() + d1_prime.clone() * u16_values.clone()));
    // 3. `n0 = 0`, `n1 = 0` - when both the context and the address remain the same, columns
    //    `d0` and `d1` at the next row should contain the delta between the old and the new clock cycle.
    ...
    // Since `n0, n1 ∈ (0, 1)` and given the fact one constraint has to hold each row we can
    // optimize the above constraint from a degree 13 polynomial to a degree 5.
    ...
    // We need to make sure that for the same context/address combination, the `v0, v1, ...` of the current row are
    // equal to the `u0, u1, ...` of the next row.
    ...
    // To use the above table in permutation checks, we need to reduce each row of the
    // memory table to a single value. This can be done like so:
    let v = beta.clone()
        + alpha.clone() * c.clone()
    ...
    // // Transition constraints need to evaluate to '0' to represent valid transitions.
    // vec![
...
fn main() {
    ...
    let beta = PrimeFieldElement::new(rng.gen());
